...
import plain2beamer.Plain2beamer;
...
public class BeamerDocument {
    ...
    public void writeInitCommendHeader() throws IOException {
        ...
        writer.write("%% Vygenerováno programem " + Plain2beamer.APP_NAME + " " + Plain2beamer.VERSION + "\n");
        writeSeparatingCommentLine();
    }
    ...
    public void writeNewListItem(String itemValue, int toPadding) throws IOException {
        fixPadding(toPadding);
        writer.write(getSpaces(padding) + "\\item" + itemValue + "%%  " + padding + "\n");
    }
    ...
    public String writeListItem(String itemText, Integer itemPadding) {
        try {
            fixPadding(itemPadding);
            writeLine(getSpaces(padding) + "\\item " + itemText);
        } catch (IOException ex) {
            return "IO chyba 9";
        }
